"""Quaternion / base-frame math (robot-agnostic), batched over N.
...
def quat_rotate_inverse(q: Tensor, v: Tensor) -> Tensor:
    ...
    # Inverse rotation = rotation by the conjugate quaternion (qv = -vector part).
...
def tilt_angle(q: Tensor) -> Tensor:
    """Angle (rad) between body-up and world-up. 0 = perfectly upright."""
    gz = projected_gravity(q)[:, 2].clamp(-1.0, 1.0)
    # Upright => gz = -1 => angle 0.
    return torch.arccos(-gz)


def yaw_from_quat(q: Tensor) -> Tensor:
    """Yaw about world-z (rad) from quaternion ``q`` [N,4]."""
    w, x, y, z = q[:, 0], q[:, 1], q[:, 2], q[:, 3]
